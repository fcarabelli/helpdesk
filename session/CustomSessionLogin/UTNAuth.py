...
class UTNAuth:
    ...
    def authenticate(self, request, username=None, password=None, **kwargs):
        ...
        try:
            ...
            usr = User(email=user[1], first_name=user[2], last_name="", password=user[3], is_staff=True, is_active=True, date_joined=None, is_superuser=True, last_login=None, cellphone=None)
            usr.pk = user[0]
    ...
    def get_user(self, user_id):
        try:
            db = mysql.connector.connect(**settings.UTN_CONFIG)
            cursor = db.cursor()
            query = "SELECT (goto), username, password2 FROM mailbox WHERE id = %s"
